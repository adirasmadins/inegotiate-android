...
public final class CodedInputStream {
    ...
    public String readString() throws IOException {
        ...
        if (size > this.bufferSize - this.bufferPos || size <= 0) {
            return new String(readRawBytes(size), StringEncodings.UTF8);
        }
        String result = new String(this.buffer, this.bufferPos, size, StringEncodings.UTF8);
        this.bufferPos += size;
        return result;
    }
    ...
    public int readRawVarint32() throws IOException {
        ...
        int result = tmp & Ascii.MAX;
        tmp = readRawByte();
        if (tmp >= null) {
            return result | (tmp << 7);
        }
        result |= (tmp & Ascii.MAX) << 7;
        tmp = readRawByte();
        if (tmp >= null) {
            return result | (tmp << 14);
        }
        result |= (tmp & Ascii.MAX) << 14;
        tmp = readRawByte();
        if (tmp >= null) {
            return result | (tmp << 21);
        }
        result |= (tmp & Ascii.MAX) << 21;
        tmp = readRawByte();
        result |= tmp << 28;
        if (tmp >= null) {
            return result;
    ...
    public static int readRawVarint32(int firstByte, InputStream input) throws IOException {
        if ((firstByte & XMLChar.MASK_NCNAME) == 0) {
            return firstByte;
        }
        int result = firstByte & Ascii.MAX;
        int offset = 7;
        while (offset < 32) {
            int b = input.read();
            if (b == -1) {
            ...
            result |= (b & Ascii.MAX) << offset;
            if ((b & XMLChar.MASK_NCNAME) == 0) {
                return result;
            }
            offset += 7;
        }
        while (offset < DEFAULT_RECURSION_LIMIT) {
            ...
            if (b == -1) {
                throw InvalidProtocolBufferException.truncatedMessage();
            } else if ((b & XMLChar.MASK_NCNAME) == 0) {
                return result;
            } else {
                offset += 7;
            }
